...
function BookViewer({ onLoginClick, pages }) {
  ...
  // --- Lógica de Escala Responsiva Dinâmica ---
  ...
  useEffect(() => {
    const updateScale = () => {
      ...
      if (screenWidth < maxWidth) {
        setBookScale(screenWidth / maxWidth);
      } else {
        setBookScale(1);
      }
    };
  ...
  const flipSpring = useSpring(flipProgress, { stiffness: 260, damping: 32, mass: 0.7, restDelta: 0.001 });
  
  // Rotação corrigida para evitar bug de face nas bordas
  const flipRotateY = useTransform(flipSpring, [0, 1], flipDir === 'left' ? [-180, 0] : [0, -180]);
  const foldBrightness = useTransform(flipSpring, [0, 0.25, 0.5, 0.75, 1], [1, 0.92, 0.78, 0.92, 1]);
  const shadowOpacity = useTransform(flipSpring, [0, 0.15, 0.5, 0.85, 1], [0, 0.45, 0.6, 0.45, 0]);
  const pageSkewY = useTransform(flipSpring, [0, 0.5, 1], [0, 2.5, 0]);
  ...
  /* -------- GSAP SCROLL TRIGGER COM MATCHMEDIA -------- */
  useGSAP(() => {
    ...
    const onUpdateShared = (self) => {
      ...
      if (scrollHintRef.current) scrollHintRef.current.classList.toggle('hidden', self.progress > 0.05);
      if (headerRef.current) headerRef.current.classList.toggle('visible', self.progress > 0.35);
    };

    mm.add("(min-width: 900px)", () => {
      gsap.set(bookSceneRef.current, { xPercent: -50, yPercent: -50, left: '72%', top: '50%', rotationY: -22, rotationZ: -6, scale: 0.82 });

      const tl = gsap.timeline({
        scrollTrigger: { trigger: '.viewport-hero', start: 'top top', end: '+=2800', scrub: 1.4, pin: true, onUpdate: onUpdateShared }
      });

      tl.to(heroTextRef.current, { opacity: 0, x: -60, duration: 0.5, ease: 'power2.in' }, 0);
      tl.to(bookSceneRef.current, { left: '50%', xPercent: -50, rotationY: 0, rotationZ: 0, scale: 1, duration: 1, ease: 'power3.inOut' }, 0.1);
      tl.add(() => { if (coverRef.current) coverRef.current.classList.add('open'); }, 0.65);
    });

    mm.add("(max-width: 899px)", () => {
      gsap.set(bookSceneRef.current, { xPercent: -50, yPercent: -50, left: '50%', top: '65%', rotationY: -15, rotationZ: -4, scale: 0.85 });

      const tl = gsap.timeline({
        scrollTrigger: { trigger: '.viewport-hero', start: 'top top', end: '+=2200', scrub: 1.4, pin: true, onUpdate: onUpdateShared }
      });

      tl.to(heroTextRef.current, { opacity: 0, y: -40, duration: 0.5, ease: 'power2.in' }, 0);
      tl.to(bookSceneRef.current, { top: '50%', rotationY: 0, rotationZ: 0, scale: 1, duration: 1, ease: 'power3.inOut' }, 0.1);
      tl.add(() => { if (coverRef.current) coverRef.current.classList.add('open'); }, 0.65);
    });

  }, { scope: containerRef, dependencies: [bookIsOpen] });
    ...
    <div ref={containerRef} className="app-container">
      ...
      <div className="viewport-hero">
        ...
        <div ref={bookSceneRef} className="book-scene" style={{ perspective: PERSPECTIVE }}>
          <div style={{ transform: `scale(${bookScale})`, transformOrigin: 'center center', position: 'relative' }}>
            <div className="book-3d-wrapper" style={{ width: BOOK_W, height: BOOK_H, transformStyle: 'preserve-3d', position: 'relative' }}>
              <div className="book-spine" style={{ height: BOOK_H }} />
              <div className="book-top" style={{ width: BOOK_W }} />
              <div className="page-stack-edge" />

              <StaticSpread
                leftPage={currentLeft} rightPage={currentRight} spreadIndex={spreadIndex}
              ...
              {isFlipping && (
                <FlippingPage
                  flipDir={flipDir} flipRotateY={flipRotateY} flipProgress={flipSpring}
                  foldBrightness={foldBrightness} shadowOpacity={shadowOpacity} pageSkewY={pageSkewY}
              ...
              <div ref={coverRef} className="book-cover-3d" style={{ zIndex: 20 }}>
              ...
              {bookIsOpen && (
                <motion.div
                  className="drag-overlay" drag="x" dragConstraints={{ left: 0, right: 0 }}
                  dragElastic={0.08} onDragEnd={handleDragEnd} style={{ zIndex: 30 }}
                />
              )}

              {bookIsOpen && (
                <div className="book-nav">
                  <button className="book-nav-btn" onClick={flipBackward} disabled={isFlipping || spreadIndex < 0}>Anterior</button>
                  <span className="page-counter">
                    {spreadIndex < 0 ? 'Capa' : `${spreadIndex * 2 + 1} – ${Math.min(spreadIndex * 2 + 2, pages.length)} / ${pages.length}`}
                  </span>
                  <button className="book-nav-btn" onClick={flipForward} disabled={isFlipping || spreadIndex >= maxSpreadIndex}>Próxima</button>
                </div>
              )}
            </div>
          </div>
    ...
    <>
      <div className="static-page" style={{ zIndex: 1 }}>
        <div className="page-face page-right">
          ...
          <span className="page-number right">{spreadIndex >= 0 ? spreadIndex * 2 + 2 : ''}</span>
        </div>
      </div>
...
/* ==============================================
   4. PÁGINA VIRANDO — FÍSICA APRIMORADA (COM FIX DO FLICKER BRANCO)
   ============================================== */
function FlippingPage({
  ...
  // FIX DE FLICKER BRANCO: Força o navegador a exibir a face correta alternando opacidade
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{
          position: 'absolute', top: 0, right: '-45%', width: '45%', height: '100%',
          background: 'linear-gradient(to right, rgba(15,8,0,0.38) 0%, rgba(15,8,0,0.15) 50%, transparent 100%)',
          opacity: shadowOpacity, backfaceVisibility: 'hidden', pointerEvents: 'none',
          zIndex: 20, borderRadius: '0 6px 6px 0',
        }}
      />
    </motion.div>
  );
      ...
      {page.elementos?.map((el, i) => {
        ...
        return (
          <div key={el.id || i} style={{
            ...
            overflow: 'hidden', borderRadius: 3, boxShadow: '0 2px 16px rgba(0,0,0,0.14)',
          }}>
            <img
              src={el.url} alt={el.legenda || 'Foto do anuário'}
              style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
              loading="lazy" draggable={false}
            />
            {el.legenda && (
              <div style={{
                position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(255,252,248,0.92)',
                padding: '5px 8px', fontSize: '10px', color: '#555', textAlign: 'center',
